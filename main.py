...
def main():
    ...
    for operacion in camino:
        if operacion == GIRAR_DERECHA:
            girar_derecha(pila_tortugas, angulo_constante)
        elif operacion == GIRAR_IZQUIERDA:
            girar_izquierda(pila_tortugas, angulo_constante)
        elif operacion == GIRAR_180:
            girar_izquierda(pila_tortugas, GIRO_COMPLETO)
        elif operacion == APILAR:
            apilar(pila_tortugas, pluma, angulo_constante, color_index)
        elif operacion == DESAPILAR:
            desapilar(pila_tortugas, pluma, dibujo)
        elif operacion == AVANZAR_F or operacion == AVANZAR_G:
            avanzar(pila_tortugas, dibujo, pluma, UNIDAD)
        elif operacion == AVANZAR_SIN_ESCRIBIR_F or operacion == AVANZAR_SIN_ESCRIBIR_G:
            avanzar_sin_escribir(pila_tortugas, dibujo, pluma, UNIDAD)
...
def apilar(pila_tortugas, pluma, angulo_constante, color_index):
    ...
    if pila_tortugas.esta_vacia():
        ...
        tortuga = Tortuga(posicion=pila_tortugas.ver_tope(
        ).get_posicion(), angulo=pila_tortugas.ver_tope().get_angulo())
